...
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, f
import gspread
...
CATEGORIES = {
"Продукты": ["rewe", "lidl", "aldi", "edeka", "penny", "netto", "kaufland", "billa", "spa
"Кафе": ["cafe", "coffee", "starbucks", "kaffee", "restaurant", "pizza", "burger", "mcdon
"Транспорт": ["bvg", "db ", "deutsche bahn", "ubahn", "s-bahn", "bus", "uber", "bolt", "t
"Здоровье": ["apotheke", "pharmacy", "arzt", "doctor", "медицин", "аптека", "psycholog",
"Фитнесс": ["fitness", "gym", "sport", "yoga", "fitnessstudio", "фитнес", "спорт"],
"Одежда": ["zara", "h&m", "hm ", "primark", "c&a", "uniqlo", "zalando", "одежда", "kleidu
"Связь плюс подписки": ["netflix", "spotify", "amazon prime", "apple", "google", "телефон
"Немецкий": ["schule", "kurs", "deutsch", "немецкий", "volkshochschule", "vhs", "ahso", "
"Подарки": ["подарок", "geschenk", "blumen", "цветы"],
"Для дома": ["ikea", "obi", "bauhaus", "hornbach", "saturn", "mediamarkt", "дом", "hausha
...
scopes = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/aut
...
transactions.append({"date": date, "description": desc, "amount": amount_eur, "catego
return transactions
...
ws.append_row(["Дата", "Месяц", "Категория", "Тип", "Стоимость", "Комментарий", "Исто
...
continue
ws.append_row([date_str, month, t["category"], "Расход", round(t["amount"], 2), comme
...
continue
transactions.append({"date": date, "description": desc, "amount": amount, "catego
...
april_rows = [r for r in data if "апр" in str(r.get("Месяц", "")) and str(r.get("Дата
expense_rows = [r for r in april_rows if r.get("Тип") == "Расход"]
total_debug = sum(float(str(r.get("Стоимость",0)).replace(",",".")) for r in expense_
...
# Google Sheets с русской локалью убирает запятую: 15,49 → 1549, 563,00 → 563
...
if not by_cat:
return
await update.message.reply_text(f" За {current_month_name} {now.year} расходов
...
await update.message.reply_text("Из какого банка этот файл?", reply_markup=InlineKeyb
return
...
reply = update_or_query.message.reply_text if hasattr(update_or_query, 'message') else up
...
context.job_queue.run_repeating(weekly_sync, interval=timedelta(weeks=1), first=timedelta
await update.message.reply_text(" Еженедельная синхронизация настроена! Каждое воскресе
def main():
