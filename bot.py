...
CATEGORIES = {
    "Продукты": [
        "rewe", "lidl", "aldi", "edeka", "penny", "netto", "kaufland",
        "billa", "spar", "metro", "продукты", "супермаркет", "market"
    ],
    "Кафе": [
        "cafe", "coffee", "starbucks", "kaffee", "restaurant", "pizza",
        "burger", "mcdonalds", "mcdonald", "kfc", "subway", "bar",
        "кафе", "ресторан", "доставка", "lieferando"
    ],
    "Транспорт": [
        "bvg", "db ", "deutsche bahn", "ubahn", "s-bahn", "bus",
        "uber", "bolt", "taxi", "mvg", "проездной", "транспорт", "tankstelle", "aral", "shell"
    ],
    "Здоровье": [
        "apotheke", "pharmacy", "arzt", "doctor", "медицин", "аптека",
        "psycholog", "линзы", "brillen", "здоровье", "dm ", "rossmann"
    ],
    "Фитнесс": [
        "fitness", "gym", "sport", "yoga", "fitnessstudio", "фитнес", "спорт"
    ],
    "Одежда": [
        "zara", "h&m", "hm ", "primark", "c&a", "uniqlo", "zalando",
        "одежда", "kleidung", "mode"
    ],
    "Связь плюс подписки": [
        "netflix", "spotify", "amazon prime", "apple", "google",
        "телефон", "симка", "vodafone", "telekom", "o2", "congstar",
        "подписка", "abo"
    ],
    "Немецкий": [
        "schule", "kurs", "deutsch", "немецкий", "volkshochschule", "vhs", "ahso"
    ],
    "Подарки": [
        "подарок", "geschenk", "blumen", "цветы"
    ],
    "Для дома": [
        "ikea", "obi", "bauhaus", "hornbach", "saturn", "mediamarkt",
        "дом", "haushalt"
    ],
    "Уход": [
        "friseur", "kosmetik", "beauty", "nails", "массаж", "уход"
    ],
}
...
def save_to_sheet(transactions: list) -> int:
    ...
    existing = ws.col_values(6)  # комментарии для проверки дублей
    added = 0
    for t in transactions:
        date_str = t["date"].strftime("%Y-%m-%d")
        month = MONTH_NAMES[t["date"].month]
        comment = t["description"]
        # простая проверка на дубли по дате+описанию
...
def parse_csv_transactions(text: str, source: str) -> list:
    """Парсит CSV выгрузку из Revolut, PayPal или Sparkasse"""
    ...
    for line in lines[1:]:
        ...
        try:
            if "revolut" in source.lower() or "started date" in header:
                # Revolut: Started Date, Completed Date, Description, Amount, Currency
                ...
                # PayPal: Date, Time, TimeZone, Name, Type, Status, Currency, Amount
                ...
                # Sparkasse: Buchungstag;Valutadatum;Auftraggeber;Buchungstext;Betrag
                parts = line.split(";")
                date = datetime.strptime(parts[0], "%d.%m.%y")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "Что я умею:\n"
        "📥 /sync — загрузить транзакции из Monobank за последние 7 дней\n"
        "📊 /summary — сводка расходов за текущий месяц\n"
        "📎 Отправь CSV файл — загружу данные из Revolut, PayPal или Sparkasse\n\n"
        "Все данные автоматически попадают в твою Google таблицу по категориям!"
    )
...
async def sync_mono(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines = [f"✅ Загружено {added} новых транзакций из {len(transactions)}!\n"]
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if parts[0] == "csv" and len(parts) == 3:
        ...
        file_id = parts[2]
        await process_csv(query, context, file_id, source)
...
async def weekly_sync(context: ContextTypes.DEFAULT_TYPE):
    """Автоматическая еженедельная синхронизация"""
    ...
    try:
        ...
        for t in transactions:
            by_cat[t["category"]] = by_cat.get(t["category"], 0) + t["amount"]
            total += t["amount"]
        lines = [f"📅 Еженедельная сводка Monobank\n"]
        lines.append(f"Загружено {added} новых транзакций\n")
        lines.append("📊 По категориям:")
        for cat, amt in sorted(by_cat.items(), key=lambda x: -x[1]):
            lines.append(f"  • {cat}: {amt:.2f} €")
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Ошибка синхронизации: {str(e)}")


async def setup_weekly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Настройка еженедельной синхронизации"""
    ...
    await update.message.reply_text(
        "✅ Еженедельная синхронизация настроена!\n"
        "Каждое воскресенье в 20:00 я буду автоматически загружать транзакции из Monobank "
        "и присылать тебе сводку 📊"
    )
